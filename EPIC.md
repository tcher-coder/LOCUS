...
**Статус (2026-07-13, вечер): MVP ЗАДЕПЛОЕН.** Stories 1–7 реализованы (бридж, доставка, лимиты), код в публичном репо https://github.com/tcher-coder/LOCUS, работает в Docker на сервере владельца (см. локальный `DEPLOYMENT.md`, не в git; эксплуатация — `RUNBOOK.md`). EPIC-2 (архив): код готов (A1/A2/A3 в bridge), но `ARCHIVE_CHANNEL_ID` ещё не подключён — включение описано в RUNBOOK. Из невыполненного: Rich Messages шлются с фолбэком на HTML (формат RichBlocks не верифицирован по живому API), `/lint` не коммитит изменения, диалог-resume теряется при рестарте (map в памяти).
...
- [ ] Проверить/установить: Node.js + `claude` CLI, Python 3.11+, `ffmpeg` (`winget install Gyan.FFmpeg`), `yt-dlp` (`pip install yt-dlp` или winget).
- [ ] `claude setup-token` → токен в `bridge/.env` (`CLAUDE_CODE_OAUTH_TOKEN`). Записать дату выпуска токена в README (живёт 1 год).
- [ ] Установить скилл видео: `npx skills add bradautomates/claude-video -g` (альтернатива — plugin marketplace, см. 2.4).
- [ ] Бот у @BotFather → `BOT_TOKEN`; узнать свой chat_id (например через @userinfobot) → `OWNER_CHAT_ID`.
- [ ] `pip install claude-agent-sdk httpx python-dotenv` (+ выбранная TG-библиотека, если используется).
- [ ] `bridge/smoke.py`: (а) вызов Agent SDK «ответь ok» проходит на подписке (проверить, что `ANTHROPIC_API_KEY` НЕ задан в окружении); (б) `ffmpeg -version` и `yt-dlp --version` доступны.
- [ ] `git init` в корне LOCUS (или отдельно в vault — см. S2).
...
- [ ] Дозаполнить структуру: `vault/wiki/index.md` (пустая навигация), `vault/wiki/log.md` (заголовок хронологии).
- [ ] Написать `vault/AGENTS.md` со схемой из раздела 2.3 и контрактами из 3.1: формат raw-документа, алгоритм ingest (5–15 страниц, [[ссылки]], log, index), правила гигиены, неизменяемость raw/, правило «/watch в режиме transcript», кап краулинга (20 страниц по умолчанию), требование маркера RESULT/SUMMARY.
- [ ] `git init` в `vault/` (если vault — отдельный репозиторий; решение: отдельный, чтобы база знаний жила независимо от кода).

Приёмка: ручной прогон в интерактивном Claude Code (cwd=vault) «проглоти этот текст по AGENTS.md» даёт корректный raw-документ, обновлённые страницы, запись в log.md и index.md.

### Story 3 — Ingest статей и сырого текста
- [ ] `prompts/ingest_article.md`: WebFetch → конспект по формату 3.1 → ingest по AGENTS.md → маркер RESULT/SUMMARY.
- [ ] `prompts/ingest_text.md`: то же без загрузки.
- [ ] Пейволл/блокировка WebFetch → честный `RESULT: NONE` с причиной (не галлюцинировать).
- [ ] Свободные инструкции: сообщение владельца целиком вставляется в промпт; сценарий «карта сайта» — найти sitemap.xml, отфильтровать по теме, кап 20 страниц, при превышении — прислать план и ждать подтверждения (см. S5 диалоговый режим).

Приёмка: 3 статьи → 3 документа + связные обновления вики; пейволл → честный отказ.

### Story 4 — Ingest видео через `/watch`
- [ ] `prompts/ingest_video.md`: `/watch <url>` в режиме `transcript` → конспект с таймкодами → ingest.
- [ ] Флаг `+frames` в сообщении → режим `efficient`.
- [ ] Прокинуть `GROQ_API_KEY` из `.env` в окружение сессии (Whisper-фолбэк). Без субтитров и без работающего фолбэка → честный отказ.

Приёмка: видео с субтитрами → конспект с таймкодами; vision-токены не тратятся в дефолте.

### Story 5 — Telegram-бридж (ядро)
- [ ] `bridge/main.py`: long polling (`getUpdates`); сообщения **только** от `OWNER_CHAT_ID`.
- [ ] Классификатор без LLM: video-URL (youtube.com/youtu.be/tiktok/vimeo/x.com…) / прочий URL / сырой текст → выбор шаблона; полный текст сообщения всегда идёт в промпт.
- [ ] Очередь FIFO в один поток; на постановку — «⏳ принято, в очереди N».
- [ ] `bridge/agent.py`: query() с `cwd=vault/`, `model="claude-sonnet-5"`, разрешённые инструменты, таймаут задачи ~10 мин; **здесь же закрыть открытый вопрос про effort** (2.5) и зафиксировать ответ в этом документе; парсинг RESULT/SUMMARY.
- [ ] Статусы стадий в чат («читаю → конспектирую → обновляю вики → готово»).
- [ ] Диалоговый режим: ответ реплаем на сообщение бота = resume той же сессии SDK (подтверждение планов краулинга, уточняющие вопросы агента).
- [ ] `git commit` в vault после успешного ingest (сообщение коммита = title документа).

Приёмка: полный цикл «ссылка → документ в ответ» E2E; чужой chat_id — отказ; две ссылки подряд обрабатываются последовательно; реплай продолжает сессию.

### Story 6 — Доставка: Rich Messages + `.md`
- [ ] `bridge/telegram_out.py`: конвертер Markdown → RichBlocks (заголовки→SectionHeading, списки→List, таблицы→Table, код→Preformatted, цитаты→Paragraph+Italic) + `sendRichMessage` сырым HTTP.
- [ ] Всегда дублировать `sendDocument` с `.md`-файлом из vault.
- [ ] Фолбэк при ошибке Rich Message: обычное сообщение (HTML parse_mode, обрезка до 4096) + файл.
- [ ] Длинные конспекты: в чат — TL;DR/тезисы, полное — файлом.

Приёмка: конспект с заголовками/таблицей/кодом рендерится в чате; файл совпадает с vault.
...
- [ ] Effort low по умолчанию; `+deep` в сообщении → medium.
- [ ] Порог длины входа: сверх него — конспект по частям или явное сокращение.
- [ ] Rate-limit подписки: понятное сообщение + автоповтор после окна сброса; ошибка одной задачи не блокирует очередь; ретрай сетевых ошибок ×1; лог в файл.
- [ ] `/stats`: задач сегодня, очередь, дата выпуска setup-token (предупреждение за 30 дней до годовщины).

### Story 8 — Команды памяти
- [ ] `/ask <вопрос>` — сессия-запрос: поиск по wiki (Grep/Read), ответ со ссылками на страницы, БЕЗ ingest (тратит лимиты).
- [ ] `/digest` — сводка последних записей log.md.
- [ ] `/lint` — ручной сеанс гигиены вики по AGENTS.md (дубли, битые [[ссылки]], устаревшее).

> Команды доступа к архиву документов (`/archive`, `/find`, `/get`, `/backfill`) — см. `EPIC-2-ARCHIVE.md`.

### Story 9 — Развёртывание 24/7
- [ ] Вариант A (старт): Windows-ПК владельца — автозапуск (Task Scheduler/NSSM), рестарт при падении.
- [ ] Вариант B (цель): Linux-VPS 1–2 ГБ RAM — systemd-unit; на сервер нужны Node.js+claude CLI, Python, ffmpeg, yt-dlp; токен переносится в `.env` (браузер не нужен); vault переносится git-клоном.
- [ ] `README.md`: запуск/обновление/перенос, продление setup-token.
